...
class BPP(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                'action_mask': spaces.Box(low=0, high=1, shape=(self.num_items * self.max_bin_size[0],), dtype=np.int8),
            }
        )
    ...
    def _get_obs(self):
        ...
        obs = dict()
        obs['states'] = np.array(self.items, dtype=np.float32)
        ...
        obs['actions'] = actions.astype(np.float32)
        self.actions = actions

        action_mask = np.ones(len(actions), dtype=np.int8)
